import React, { useState, useEffect, useContext } from 'react'
import { UserContext } from './App'

const readFileAsDataURL = (file) =>
  new Promise(resolve => {
    const reader = new FileReader()

    reader.onload = (event) => {
      resolve(event.target.result)
    }

    reader.readAsDataURL(file)
  })

const resizeImage = (imageURL, canvas, maxHeight) =>
  new Promise(resolve => {
    const image = new Image()

    image.onload = () => {
      const context = canvas.getContext('2d')

      if (image.height > maxHeight) {
        image.width *= maxHeight / image.height
        image.height = maxHeight
      }

      context.clearRect(0, 0, canvas.width, canvas.height)
      canvas.width = image.width
      canvas.height = image.height

      context.drawImage(image, 0, 0, image.width, image.height)

      resolve(canvas.toDataURL('image/jpeg'))
    }

    image.src = imageURL
  })

/**
 * A custom <input> that dynamically reads and resizes image files before
 * submitting them to the server as data URLs. Also, shows a preview of the image.
 */
/* interface ImageInputProps {
  className: string,
  name: string,
  maxHeight: number
} */
export default function ImageInput(props) {
  const { maxHeight, className, name } = props
  const [image, setImage] = useState('')
  const canvas = document.createElement('canvas')
  const [fileInput, setFileInput] = useState(null);
  const { contact } = useContext(UserContext)

  const handleFileChange = (event) => {
    const file = event.target.files[0]

    if (file && file.type.match(/^image\//)) {
      readFileAsDataURL(file).then(originalURL => {
        resizeImage(originalURL, canvas, maxHeight).then(url => {
          setImage(url)
        })
      })
    } else {
      setImage('')
    }
  }

  const handleFormReset = () => {
    setImage('')
  }

  useEffect(() => {
    if (contact != null) {
      setImage(contact.avatar)
    }
    return () => {
      if (fileInput != null)
        fileInput.removeEventListener('reset', handleFormReset)
    }
  }, []);

  useEffect(() => {
    if (fileInput != null)
      fileInput.addEventListener('reset', handleFormReset)
  }, [fileInput])

  const style = {
    position: 'relative'
  }

  if (image) {
    style.backgroundImage = `url("${image}")`
    style.backgroundRepeat = 'no-repeat'
    style.backgroundPosition = 'center'
    style.backgroundSize = 'cover'
  }

  return (
    <div className={className} style={style}>
      <input type="hidden" name={name} value={image} />
      <input
        ref={(el) => { setFileInput(el) }}
        type="file"
        onChange={handleFileChange}
        style={{
          position: 'absolute',
          top: 0,
          left: 0,
          width: '100%',
          height: '100%',
          opacity: 0
        }}
      />
    </div>
  )
}